 ...
 * no tenía ninguna zona activa, mientras Clientes le mostraba dos. La causa no estaba en el
 * servidor —la API devolvía las dos zonas activas— sino en que <b>por este camino nadie las
...
describe('Servicios · llegando con un cliente enlazado', () => {
  ...
  function llegar(zones: readonly { idClientZone: string; name: string; active: boolean }[]) {
    ...
    http.expectOne((request) => request.url === `/api/v1/clients/${CLIENTE}`)
      .flush({ idClient: CLIENTE, idOrganization: 'org-a', legalName: 'Cliente con zonas', tradeName: 'awddaw' });
    fixture.detectChanges();

    http.match((request) => request.url === '/api/v1/services')
      .forEach((request) => request.flush({ items: [], totalCount: 0, page: 1, pageSize: 20, totalPages: 1 }));

    http.expectOne((request) => request.url === `/api/v1/clients/${CLIENTE}/zones`).flush(zones);
  ...
  it('pide las zonas del cliente, aunque todavía no tenga ni un servicio', () => {
    llegar([{ idClientZone: 'zone-1', name: 'Zona de prueba', active: true }]);

    // La comprobación de verdad la hace `http.verify()` en el afterEach: si la petición de zonas no
    // se hubiera hecho, el `expectOne` de arriba habría fallado.
    expect(raiz().textContent).not.toContain('no tiene ninguna zona activa');
  });

  it('con una zona activa, «Nuevo servicio» abre el alta en vez de negarla', () => {
    llegar([{ idClientZone: 'zone-1', name: 'Zona de prueba', active: true }]);
  ...
  /** Y cuando de verdad no hay zona activa, el aviso sigue siendo el correcto. */
  it('sin ninguna zona activa, sí lo dice', () => {
    llegar([{ idClientZone: 'zone-1', name: 'Zona dada de baja', active: false }]);
    ...
    expect(raiz().textContent).toContain('no tiene ninguna zona activa');
  });
});
